...
# Test RC (improved model)
print("=" * 60)
print("TESTING RC (improved)")
print("=" * 60)
policy, kind, path = load_rl_policy(".", 1.0)
print(f"Loaded {kind} from {path}")
obs, _ = env.reset()
policy.reset()
start_x = get_base_x(env)
...
for step in range(500):
    ...
    action = np.clip(action, env.action_space.low, env.action_space.high)
    obs, reward, term, trunc, _ = env.step(action)
    total_reward_rc += reward
    steps_rc = step + 1
    if term or trunc:
        break

end_x = get_base_x(env)
distance_rc = end_x - start_x
print(f"RC - Distance: {distance_rc:.4f}m, Total Reward: {total_reward_rc:.2f}, Steps: {steps_rc}")
print()
...
print("COMPARISON")
print("=" * 60)
ratio = distance_rc / distance_cpg if distance_cpg > 0 else 0
print(f"CPG moves:     {distance_cpg:.4f}m (reward: {total_reward_cpg:.2f})")
print(f"RC moves:      {distance_rc:.4f}m (reward: {total_reward_rc:.2f})")
print()
if distance_cpg > 0:
    print(f"RC is {ratio*100:.1f}% of CPG distance")
    if ratio >= 0.9:
        print("✅ RC performs comparably to CPG!")
    elif ratio >= 0.5:
        print("⚠️  RC shows reduced movement vs CPG")
    elif ratio >= 0.2:
        print("✅ RC shows notable forward movement")
    else:
        print("⚠️  RC movement is limited")
else:
    print(f"CPG moved backward, RC clearly superior")
